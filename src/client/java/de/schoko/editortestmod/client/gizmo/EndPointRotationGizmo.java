...
import de.schoko.editortestmod.client.core.hitboxes.LineRing;
...
public class EndPointRotationGizmo implements Gizmo {
	...
	public void draw(RenderContext renderContext, EditorObject target) {
		...
		EditorTestModClient.addDebugString("Yaw", Math.round(Math.toDegrees(endpoint.getYaw())));
		EditorTestModClient.addDebugString("Pitch", Math.round(Math.toDegrees(endpoint.getPitch())));
		EditorTestModClient.addDebugString("Roll", Math.round(Math.toDegrees(endpoint.getRoll())));
