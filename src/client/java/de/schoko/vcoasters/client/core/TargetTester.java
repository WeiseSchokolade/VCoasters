...
public interface TargetTester {
	...
	static Vec3 getPlayerViewDirection() {
		return new Vec3(VCoastersClient.instance.getLastCamera().forwardVector());
	}
	...
	static Vec3 getMouseViewDirection() {
		...
		Matrix4f matrix4f = new Matrix4f(VCoastersClient.instance.getLastProjectionMatrix());
		matrix4f.invert();
		...
		view.rotate(VCoastersClient.instance.getLastCamera().rotation());
		//VCoasters.lastResult = rawVec;
		return new Vec3(view).normalize();
	}

	static Vec3 getEyePosition() {
		return VCoastersClient.instance.getLastCamera().position();
	}
